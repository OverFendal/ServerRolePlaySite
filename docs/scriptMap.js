document.addEventListener("DOMContentLoaded", () => {
    ...
    regions.forEach(region => {
        region.addEventListener("click", (e) => {
            ...
            tooltipText.innerHTML = `<strong>${actual_name}</strong> ${a}`;
            tooltip.style.display = "block";
            const tooltipPadding = 10; // отступ от края экрана
            const tooltipWidth = 350; // предполагаемая ширина тултипа (можно уточнить)
            const tooltipHeight = 200; // предполагаемая высота тултипа (можно уточнить)
            ...
            // Если тултип выйдет за правый край — сдвигаем влево от курсора
            if ((left + tooltipWidth) > pageWidth) {
                left = e.pageX - tooltipWidth - 15;
                if (left < tooltipPadding) left = tooltipPadding;
            }

            // Если тултип выйдет за нижний край — сдвигаем вверх
...
// Обработчик клика вне тултипа
    document.addEventListener('click', (e) => {
        // Если клик был не по тултипу и не по региону
        if (!tooltip.contains(e.target) && !e.target.closest('.highlight_borders')) {
            tooltip.style.display = "none";
        }
    });
});